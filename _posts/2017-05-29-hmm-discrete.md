...
The Maximum Likelihood Estimation (MLE) of the model parameter $$\lambda$$ for complete data $$Y$$ and hidden state $$S$$ is; 
...
We provide a solution for each term. Considering the first term $$Q(\hat{\pi} \mid \pi)$$ we define the following auxiliary function for $$\pi _i$$ as:
